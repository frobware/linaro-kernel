...
static HI_S32 hivdec_probe(struct platform_device * pltdev);
...
static inline HI_S32 vdec_init_with_vfmw(vfmw_event_handler handler)
{
    ...
    stInitParam.VdecCallback = handler;
    
	return (g_stOmxFunc.pVfmwFunc->pfnVfmwInitWithOperation)(&stInitParam);
}
    ...
    { 
		...
		case EVNT_LAST_FRAME:
        {
            ...
            if (REPORT_LAST_FRAME_FAIL != ptemp[0])
            {
                ...
                OmxPrint(OMX_INFO, "Last Image report: %d\n", pchan->last_frame_info[1]);
            }
            else 
            {
       	...
       	case EVNT_NO_STREAM:
       	{
       		ret = channel_handle_no_stream(pchan);
       		break;
       	}
