                          ...
                          AO_ALSA_I2S_Param_S* pstAoI2sParam, HI_BOOL bResume)
{
    ...
    {
        if (HI_SUCCESS != SndOpCreate(&hSndOp,&pstAttr->stOutport[u32PortNum],pstAoI2sParam, bResume))
        {
        ...
        {
            /* Get gpio functions */
            HI_DRV_MODULE_GetFunction(HI_ID_GPIO, (HI_VOID**)&pCard->pstGpioFunc);
            ...
            {
                pCard->stMuteDisableTimer.expires = (jiffies + msecs_to_jiffies(AO_SND_MUTE_BOOT_DISABLE_TIMEMS));
            }
