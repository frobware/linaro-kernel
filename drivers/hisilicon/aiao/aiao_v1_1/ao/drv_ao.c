...
HI_S32 AOGetSndDefOpenAttr(AO_SND_OpenDefault_Param_S *pstSndDefaultAttr)
{
    ...
    pstSndDefaultAttr->stAttr.enSampleRate = HI_UNF_SAMPLE_RATE_48K;
#else
    pstSndDefaultAttr->stAttr.u32PortNum = 1;
    pstSndDefaultAttr->stAttr.stOutport[0].enOutPort = HI_UNF_SND_OUTPUTPORT_DAC0;
    pstSndDefaultAttr->stAttr.stOutport[0].unAttr.stDacAttr.pPara = HI_NULL;
    pstSndDefaultAttr->stAttr.enSampleRate = HI_UNF_SAMPLE_RATE_48K;
#endif
                       ...
                       HI_BOOL bAlsaTrack, AO_BUF_ATTR_S *pstBuf, HI_HANDLE hTrack)
{
    ...
    if(pCard)
    {
        ...
        if (HI_SUCCESS == s32Ret)
        {
        	if(HI_TRUE !=bAlsaTrack)	//No alsa track
        	{
				HI_INFO_AO("set track %d, bAllTrackMute=%d\n", hTrack, pCard->bAllTrackMute);
				return TRACK_SetMute(pCard, hTrack, pCard->bAllTrackMute);
        	}
        }
        return s32Ret;
    }
...
HI_S32 AO_Track_AttachAi(HI_U32 u32TrackID, HI_HANDLE hAi)
{
    ...
    if (pCard)
    {
        ...
        Ret = AI_GetAttr(hAi,&stAiAttr);
        if(HI_SUCCESS != Ret)
        {
            HI_ERR_AO("call AI_GetAttr failed!\n");
            return Ret;
        }

        Ret = TRACK_SetPcmAttr(pCard, hTrack, &stAiaoBuf, &stAiAttr);
        if(HI_SUCCESS != Ret)
        {
...
static HI_S32 SndProcSaveThread(void *Arg)
{
	...
	CHECK_AO_NULL_PTR(stThreadArg.fileHandle);
	...
	if (HI_SUCCESS == AO_Cast_AllocHandle(&hHandle, stThreadArg.fileHandle, &stCastParam.stCastAttr))
	{
	...
	while(HI_TRUE == bSaveThreadRunFlag) // NO !kthread_should_stop() to avoid dead lock
	{
		...
		if(HI_SUCCESS == s32Ret)
		{
			...
			else
			{
				...
				if(stThreadArg.fileHandle)
					HI_DRV_FILE_Write(stThreadArg.fileHandle, (HI_S8 *)(stCastInfo.u32KernelVirtAddr + stCastData.u32DataOffset) , u32PcmSize);
				else
				{
...
Close_File:
	if(stThreadArg.fileHandle)
	{
		HI_DRV_FILE_Close(stThreadArg.fileHandle);
	}
	s32Ret = AO_Snd_Kclose(stThreadArg.enSound, stThreadArg.devfileHandle);
	if(HI_SUCCESS != s32Ret)
	{
		HI_ERR_AO("AO_Snd_Kclose %d failed \n", (HI_U32)stThreadArg.enSound);
	}
	s32Ret = AO_DRV_Krelease(stThreadArg.devfileHandle);
	if(HI_SUCCESS != s32Ret)	
	{
...
HI_S32 SND_WriteProc(SND_CARD_STATE_S *pCard, SND_DEBUG_CMD_CTRL_E enCmd)
{
	...
	if(SND_DEBUG_CMD_CTRL_START == enCmd && pCard->enSaveState == SND_DEBUG_CMD_CTRL_STOP)
	{
        ...
        snprintf(szPath, sizeof(szPath), "%s/sound%d_%.2d.pcm", szPath, (HI_U32)enSound, pCard->u32SaveCnt);
        ...
        for (i = 0; i < AO_MAX_TOTAL_TRACK_NUM; i++)
        {
            if (atomic_read(&s_stAoDrv.astTrackEntity[i].atmUseCnt))
            {
                ...
                if(s_stAoDrv.astTrackEntity[i].stSuspendAttr.bAttAi)
                {
                    ...
                    {
                        s32Ret = AI_SetEnable(s_stAoDrv.astTrackEntity[i].stSuspendAttr.hAi, HI_FALSE);
                        if (HI_SUCCESS != s32Ret)
                        {
                    ...
                    if(HI_TRUE == bAiEnable)
                    {
                        s32Ret = AI_SetEnable(s_stAoDrv.astTrackEntity[i].stSuspendAttr.hAi, HI_TRUE);
                        if (HI_SUCCESS != s32Ret)
                        {
                            HI_FATAL_AO("AI SetEnable failed!!\n");
                            up(&g_AoMutex);
