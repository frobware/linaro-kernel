...
extern HI_S32 AO_Track_Start(HI_U32 u32TrackID);
...
#define AO_DELAY_FOR_AI_TIME 20		//20ms
...
static HI_S32 AICreateChn(HI_UNF_AI_E enAiPort, HI_UNF_AI_ATTR_S *pstAttr, HI_BOOL bAlsa, HI_VOID *pAlsaPara, AI_CHANNEL_STATE_S *state)
{
    ...
    HAL_AIAO_P_GetBorardRxI2SDfAttr(u32BoardI2sNum, &pstAttr->unAttr.stI2sAttr.stAttr, &enPort, &stHwPortAttr);
#endif
    u32BufSize = pstAttr->u32PcmSamplesPerFrame*u32FrameSize*pstAttr->u32PcmFrameMaxNum;
    stHwPortAttr.stBufConfig.u32PeriodNumber = u32BufSize/stHwPortAttr.stBufConfig.u32PeriodBufSize;
#ifdef HI_ALSA_AI_SUPPORT
    if(bAlsa == HI_TRUE)
    {
        stHwPortAttr.bExtDmaMem = HI_TRUE;
...
static HI_S32 AI_Create(AI_Create_Param_S_PTR pstAi,HI_HANDLE hAi, struct file *pstFile)
{
    ...
    CHECK_AI_MCLKDIV(pstAttr->unAttr.stI2sAttr.stAttr.enMclkSel);
...
HI_S32 AI_SetEnable(HI_HANDLE hAi, HI_BOOL bEnable)
{
    HI_S32 Ret;
    AI_CHANNEL_STATE_S *state = HI_NULL;
#if  0           
    ...
    if(bEnable)
    {
        ...
        else
        {
            ...
            if(state->bAttach)
            {
                Ret = AO_Track_GetDelayMs(state->hTrack, &u32DelayMs);
                if(HI_SUCCESS != Ret)
                {
                    msleep(AO_DELAY_FOR_AI_TIME);  //avoid buff empty interrupt
                }
                else
                {
                    if(u32DelayMs <= AO_DELAY_FOR_AI_TIME)
                    {
                        msleep(AO_DELAY_FOR_AI_TIME - u32DelayMs);  //avoid buff empty interrupt
                    }
                    else
                    {
                    }
...
static HI_S32 AI_SetAttr(HI_HANDLE hAi, HI_UNF_AI_ATTR_S *pAiAttr)
{
    ...
    if(AI_CHANNEL_STATUS_START == state->enCurnStatus)
    {
        HI_ERR_AI("AI is not disable,cannot set attr!\n");
    ...
    if(AI_CMD_CTRL_START == pAistate->enSaveState)
    {
        if(16 == pstFrame->s32BitPerSample || 32 == pstFrame->s32BitPerSample)
        {
            if(pAistate->fileHandle)
            {
                ...
                HI_U32 u32FrameSize = AUTIL_CalcFrameSize(pstFrame->u32Channels, pstFrame->s32BitPerSample);;
                HI_DRV_FILE_Write(pAistate->fileHandle, (HI_U8*)pAistate->stAiBuf.u32KernelVirBaseAddr, pstFrame->u32PcmSamplesPerFrame * u32FrameSize);
            }
        }
    }
    return;
...
static HI_S32 AI_AcquireFrame(HI_HANDLE hAi, HI_UNF_AO_FRAMEINFO_S *pstFrame)
{
    ...
    if(!state->stAiBuf.u32Read)         //do not need read data from port buff
    {
        ...
        if(HI_UNF_I2S_BIT_DEPTH_16 == state->stSndPortAttr.unAttr.stI2sAttr.stAttr.enBitDepth)
        {
...
HI_S32 AI_WriteProc(HI_U32 u32Ai, AI_CMD_CTRL_E enCmd)
{
    ...
    if(AI_CMD_CTRL_START == enCmd && AI_CMD_CTRL_STOP == state->enSaveState)
    {
        ...
        snprintf(szPath, sizeof(szPath), "%s/ai%d_%.2d.pcm", szPath, u32Ai, state->u32SaveCnt);
...
HI_S32 HI_DRV_AI_Create(HI_UNF_AI_E enAiPort, HI_UNF_AI_ATTR_S *pstAttr, HI_HANDLE *phandle)
{
    ...
    Ret = AI_AllocHandle(&hAi);
    if(HI_SUCCESS != Ret)
    {
        HI_ERR_AI("Alloc Ai Handle failed!");
        return Ret;
    }
    Ret = AI_Create(&stAiParam, hAi, &g_file);
    {
        HI_ERR_AI("Ai Create failed!");
        return Ret;
    }
    return Ret;
...
{
    CHECK_AI_CHN_OPEN(hAi);
    return AI_SetEnable(hAi, bEnable);
}
