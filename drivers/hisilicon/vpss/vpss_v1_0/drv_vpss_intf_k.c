...
static VPSS_EXPORT_FUNC_S s_VpssExportFuncs =
{
    ...
    .pfnVpssResume   = VPSS_CTRL_Resume,
    .pfnVpssSuspend  = VPSS_CTRL_Suspend,
};
...
HI_S32 HI_DRV_VPSS_PutImage(VPSS_HANDLE hVPSS,HI_DRV_VIDEO_FRAME_S *pstImage)
{
    ...
    bSupport = VPSS_INST_CheckImage(pstImage);
    if (bSupport == HI_FALSE)
    {
        VPSS_FATAL("Put in image can't be processed\n");
        return HI_FAILURE;
    }
    //VPSS_OSAL_DownLock(&(pstInstance->stInstLock));
...
HI_S32 HI_DRV_VPSS_UpdatePqData(HI_U32 u32UpdateType, PQ_PARAM_S* pstPqParam)
{
    ...
    else if ((PQ_CMD_VIRTUAL_SHARP_HD_LUMA <= u32UpdateType) && (PQ_CMD_VIRTUAL_SHARP_GP1_CHROMA >= u32UpdateType))
    {
        VPSS_ALG_SetPqDebug(HI_TRUE);    
