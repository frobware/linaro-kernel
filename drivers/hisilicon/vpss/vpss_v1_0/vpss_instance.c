...
HI_S32 VPSS_INST_AddUndoImage(VPSS_INSTANCE_S *pstInstance)
{
    ...
    if(pstInstance->eSrcImgMode == VPSS_SOURCE_MODE_VPSSACTIVE)
    {
        ...
        }
        {
            ...
            evt.frame = pstEmpty1stImage->stSrcImage.u32FrameIndex;
            evt.handle = pstEmpty1stImage->stSrcImage.hTunnelSrc;
            evt.time = TmpTime; 
                                ...
                                HI_DRV_VPSS_CFG_S *pstVpssCfg)
{
    ...
    pstInstUsrcCfg->bAlwaysFlushSrc = pstVpssCfg->bAlwaysFlushSrc;
    pstInstUsrcCfg->bProgRevise = pstVpssCfg->bProgRevise;
     VPSS_OSAL_UpSpin(&(pstInstance->stUsrSetSpin), &flags);
                                ...
                                HI_DRV_VPSS_PORT_CFG_S *pstPortCfg)
{
    ...
    pstPortCfg->stScreen = pstUsrPortCfg->stScreen;
    pstPortCfg->stCustmAR = pstUsrPortCfg->stCustmAR;
    pstPortCfg->bInterlaced = pstUsrPortCfg->bInterlaced;
    pstPortCfg->b3Dsupport = pstUsrPortCfg->b3Dsupport;
    pstPortCfg->enRotation = pstUsrPortCfg->enRotation;
    pstPortCfg->bHoriFlip = pstUsrPortCfg->bHoriFlip;
    pstPortCfg->bVertFlip = pstUsrPortCfg->bVertFlip;
                                ...
                                HI_DRV_VPSS_PORT_CFG_S *pstPortCfg)
{
    ...
    pstUsrPortCfg->enRotation = pstPortCfg->enRotation;
    pstUsrPortCfg->bHoriFlip = pstPortCfg->bHoriFlip;
    pstUsrPortCfg->bVertFlip = pstPortCfg->bVertFlip;
     VPSS_OSAL_UpSpin(&(pstInstance->stUsrSetSpin),&flags);
    #if 0
                                ...
                                HI_DRV_VPSS_PORT_CFG_S *pstVpssPortCfg)
{
    ...
    if(pstVpssPortCfg->eFormat == HI_DRV_PIX_FMT_NV12 
		...
		|| pstVpssPortCfg->eFormat == HI_DRV_PIX_FMT_ARGB8888
		|| pstVpssPortCfg->eFormat == HI_DRV_PIX_FMT_ABGR8888)
    {
...
HI_S32 VPSS_INST_CorrectProgInfo(VPSS_INSTANCE_S *pstInstance,HI_DRV_VIDEO_FRAME_S *pImage)
{
    ...
    */
    ...
    if (pstInstance->bProgRevise == HI_FALSE)
    {
	    pImage->bProgressive = HI_TRUE;
        return HI_SUCCESS;
    }
...
HI_S32 VPSS_INST_SyncUsrCfg(VPSS_INSTANCE_S * pstInstance)
{
    ...
    if(pstInstance->u32IsNewCfg)
    {
        ...
        pstInstance->bProgRevise = pstInstUsrcCfg->bProgRevise;
