...
HI_S32 VPSS_HAL_Init(HI_U32 u32HalVersion,VPSS_HAL_CAP_S *pstHalCaps)
{
    ...
    {
        VPSS_FATAL("Alloc VPSS_RegBuf_0 Failed\n");
        return HI_FAILURE;
    }
    ...
    if (s32Ret != HI_SUCCESS)
    {
        VPSS_FATAL("Alloc VPSS_RegBuf_1 Failed\n");
    ...
    s32Ret = HI_DRV_MMZ_AllocAndMap("VPSS_RegBuf_2", HI_NULL,sizeof(VPSS_REG_S),
        	                                              0, &(stHalInfo.stRegBuf[2]));
    if (s32Ret != HI_SUCCESS)
    {
        VPSS_FATAL("Alloc VPSS_RegBuf_2 Failed\n");
        HI_DRV_MMZ_UnmapAndRelease(&(stHalInfo.stRegBuf[0]));
    ...
    VPSS_REG_SetMisCellaneous((HI_U32)PhyReg,DEF_LOGIC_MISCELLANEOUS);
  
    return HI_SUCCESS;
}

HI_S32 VPSS_HAL_Reset(HI_VOID)
{
    VPSS_REG_ReSetCRG(); 
    ...
    VPSS_HAL_SetMisCellaneous(DEF_LOGIC_MISCELLANEOUS);
    return HI_SUCCESS;
}
...
HI_S32 VPSS_HAL_GetClockState(HI_BOOL *bOpened)
{
    VPSS_REG_GetClockState(bOpened);
    return HI_SUCCESS;
}
...
HI_S32 VPSS_HAL_GetRegPort(VPSS_ALG_PortCFG_S *pstAuPortCfg)
{
    ...
    if(pstAuPortCfg->bFidelity == HI_TRUE)
    {
        ...
        {
            VPSS_FATAL("Can't get Fidelity outport\n");
            return HI_FAILURE;
        }
        
    }
    else if (pstAuPortCfg->stDstFrmInfo.ePixFormat == HI_DRV_PIX_FMT_ARGB8888
            || pstAuPortCfg->stDstFrmInfo.ePixFormat == HI_DRV_PIX_FMT_ARGB8888)
    {
        if(stHalInfo.u32RegPortAlloc[VPSS_REG_HD] != 1)
        {
            pstAuPortCfg->eRegPort = VPSS_REG_HD;
            stHalInfo.u32RegPortAlloc[VPSS_REG_HD] = 1;
            return HI_SUCCESS;
        }
        else
        {
            VPSS_FATAL("Can't get ARGB8888 outport\n");
            return HI_FAILURE;
        }
    }
    else
...
HI_S32 VPSS_HAL_SetHalCfg(VPSS_ALG_CFG_S *pstAlgCfg,HI_U32 u32NodeID)
{
	...
	s32Ret = VPSS_HAL_SetPreZmeCfg(pstAlgCfg,u32NodeID);
    ...
    for (u32Count = 0; u32Count < DEF_HI_DRV_VPSS_PORT_MAX_NUMBER; u32Count ++)
    {
        ...
        if (pstFrmCfg->u32Height != 0)
        {
            ...
            s32Ret = VPSS_HAL_SetMirrorCfg(u32Count, pstAuPortCfg->eRegPort, pstAlgCfg,u32NodeID);
            #if DEF_TUNNEL_EN
			...
			#if 1
			if (pstAuPortCfg->eRegPort == VPSS_REG_HD
			    && (pstAuPortCfg->stDstFrmInfo.ePixFormat == HI_DRV_PIX_FMT_ARGB8888
...
HI_S32 VPSS_HAL_SetMisCellaneous(HI_U32 u32Value)
{
    VPSS_REG_SetMisCellaneous((HI_U32)PhyReg,u32Value);
                            ...
                            VPSS_ALG_CFG_S *pstAlgCfg,HI_U32 u32NodeID)
{
    ...
    if (pstFrmCfg->u32Height != 0)
    {
        ...
        {
            VPSS_REG_SetZmeEnable(u32AppVAddr,ePort,REG_ZME_MODE_ALL,HI_FALSE);
        }
        else
        {
        ...
        if (pstZmeCfg->enZmeYCFmtIn == ALG_PIX_FORMAT_SP420)
        {
            VPSS_REG_SetZmeInFmt(u32AppVAddr,ePort,HI_DRV_PIX_FMT_NV21);
        }
        else
        {
            VPSS_REG_SetZmeInFmt(u32AppVAddr,ePort,HI_DRV_PIX_FMT_NV61_2X1);
        }
        
        if (pstZmeCfg->enZmeYCFmtOut== ALG_PIX_FORMAT_SP420)
        {
            VPSS_REG_SetZmeOutFmt(u32AppVAddr,ePort,HI_DRV_PIX_FMT_NV21);
        }
        else
        {
            VPSS_REG_SetZmeOutFmt(u32AppVAddr,ePort,HI_DRV_PIX_FMT_NV61_2X1);
        }
...
HI_S32 VPSS_HAL_SetCurTunl(VPSS_ALG_CFG_S *pstAlgCfg,HI_U32 u32NodeID)
{
    ...
    if(pstSrcImg->u32TunnelAddr != 0)
    {
        u32CurTunlRdInterval = 10;
        u32CurTunlAddr = pstSrcImg->u32TunnelAddr;
...
HI_S32 VPSS_HAL_SetRotationCfg(VPSS_ALG_CFG_S *pstAlgCfg,HI_U32 u32PortID,HI_U32 u32NodeID)
{
    ...
    s32Ret = VPSS_HAL_SetRotation(pstAuPortCfg->enRotation,u32NodeID);
...
HI_S32 VPSS_HAL_SetMirrorCfg(HI_U32 u32PortID,VPSS_REG_PORT_E ePort,
                            VPSS_ALG_CFG_S *pstAlgCfg,HI_U32 u32NodeID)
{
    HI_U32 u32AppVAddr;
    VPSS_ALG_PortCFG_S *pstAuPortCfg;
    u32AppVAddr = (HI_U32)AppReg[u32NodeID];
    pstAuPortCfg = &(pstAlgCfg->stAuPortCfg[u32PortID]);
    VPSS_REG_SetPortMirrorEn(u32AppVAddr,ePort, pstAuPortCfg->bNeedMirror);    

    return HI_SUCCESS;
}   
HI_S32 VPSS_HAL_SetPreZmeCfg(VPSS_ALG_CFG_S *pstAlgCfg,HI_U32 u32NodeID)
{
    HI_U32 u32AppVAddr;

    u32AppVAddr = (HI_U32)AppReg[u32NodeID];

    VPSS_REG_SetPreZme(u32AppVAddr,
                        pstAlgCfg->stAuTunnelCfg.enHorPreZme,
                        pstAlgCfg->stAuTunnelCfg.enVerPreZme);
    return HI_SUCCESS;
}
