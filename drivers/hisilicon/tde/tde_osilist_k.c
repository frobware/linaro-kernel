                           ...
                           TDE_NOTIFY_MODE_E enNotiType)
{
    ...
    if (s32Ret == HI_SUCCESS)
    {
        ...
        s_pstTDEOsiJobList[enListType]->pstJobFinished = HI_NULL;
    }
...
HI_VOID TdeOsiListNodeComp(TDE_LIST_TYPE_E enListType)
{
    ...
    if(u32RunningSwNodeAddr ==(HI_U32)s_pstTDEOsiJobList[enListType]->pstJobFinished)
    {
        tde_osr_enableirq();
        return;
    }
    ...
    if (!bWork)
    {
        s_pstTDEOsiJobList[enListType]->pstJobFinished = (HI_VOID *)u32RunningSwNodeAddr;
    }
    else
    {
