...
HI_S32 DRV_DISP_SetChan0Option(PQ_PARAM_S* pstPqParam, OPTION_MODE_E eOptionMode)
{
    HI_S32  s32Ret = HI_FAILURE;

    HI_DRV_DISP_COLOR_SETTING_S stCS;
    
    HI_S32 s32ColorTempMode = 0;
    
    PQ_COLOR_TEMP_COEF_S stColorTemp;
    PQ_OPT_CHAN0_S *pstOptionChan0;
    pstOptionChan0 = &(pstPqParam->stPqOption.stWorkPqOptCoef.stOptChan0[eOptionMode]);
    stColorTemp = pstPqParam->stPQCoef.stColorTempCoef;
    
    s32Ret = DISP_GetColor(HI_DRV_DISPLAY_0, &stCS);
    ...
    stCS.u32Bright = pstOptionChan0->u32Brightness;
    stCS.u32Hue = pstOptionChan0->u32Hue;
    stCS.u32Satur = pstOptionChan0->u32Saturation;
    stCS.u32Contrst = pstOptionChan0->u32Contrast;

    s32ColorTempMode = pstOptionChan0->u32colortemperature;

    switch(s32ColorTempMode)    
    {
	    case TEMP_MODE_COLD:
		{
	        stCS.u32Kb = stColorTemp.stPqTempModeCold.u32Kb;
	        stCS.u32Kg = stColorTemp.stPqTempModeCold.u32Kg;
	        stCS.u32Kr = stColorTemp.stPqTempModeCold.u32Kr;
	        break;
		}
	    case TEMP_MODE_MIDDLE:
		{
	        stCS.u32Kb = stColorTemp.stPqTempModeMiddle.u32Kb;
	        stCS.u32Kg = stColorTemp.stPqTempModeMiddle.u32Kg;
	        stCS.u32Kr = stColorTemp.stPqTempModeMiddle.u32Kr;
	        break;
		}
	    case TEMP_MODE_WARM:
		{
	        stCS.u32Kb = stColorTemp.stPqTempModeWarm.u32Kb;
	        stCS.u32Kg = stColorTemp.stPqTempModeWarm.u32Kg;
	        stCS.u32Kr = stColorTemp.stPqTempModeWarm.u32Kr;
	        break;
		}
	    default:
	        break;            
    ...
    s32Ret |= DISP_SetColor(HI_DRV_DISPLAY_0, &stCS);

    if(GAMMA_PQ_MODE_CLOSE == pstOptionChan0->u32GammaMode)
    {
        s32Ret |= DISP_SetGammaCtrl(HI_DRV_DISPLAY_0, pstOptionChan0->u32GammaMode, HI_FALSE);
    }
    else
    {
        s32Ret |= DISP_SetGammaCtrl(HI_DRV_DISPLAY_0, pstOptionChan0->u32GammaMode, HI_TRUE);
    }
    

    return s32Ret;
}
HI_S32 DRV_DISP_SetChan1Option(PQ_PARAM_S* pstPqParam,OPTION_MODE_E eOptionMode)
{
    HI_S32  s32Ret = HI_FAILURE;        
    HI_DRV_DISP_COLOR_SETTING_S stCS;
    WIN_EXPORT_FUNC_S* pstVoFuncs = HI_NULL;   
    
    PQ_OPT_CHAN1_S *pstOptionChan1;
    HI_S32 s32ColorTempMode = 0;
    PQ_COLOR_TEMP_COEF_S stColorTemp;

    pstOptionChan1 = &(pstPqParam->stPqOption.stWorkPqOptCoef.stOptChan1[eOptionMode]);
    stColorTemp = pstPqParam->stPQCoef.stColorTempCoef;
    (HI_VOID)HI_DRV_MODULE_GetFunction(HI_ID_VO, (HI_VOID**)&pstVoFuncs); 
    ...
    s32Ret = DISP_GetColor(HI_DRV_DISPLAY_1, &stCS);
    ...
    stCS.u32Bright = pstOptionChan1->u32Brightness;
    stCS.u32Hue = pstOptionChan1->u32Hue;
    stCS.u32Satur = pstOptionChan1->u32Saturation;
    stCS.u32Contrst = pstOptionChan1->u32Contrast;
    //stCS.u32Kb = pstOptionChan1->u32colortemperature;
    //stCS.u32Kg = pstOptionChan1->u32colortemperature;
    //stCS.u32Kr = pstOptionChan1->u32colortemperature;

    s32ColorTempMode = pstOptionChan1->u32colortemperature;
    ...
    s32Ret |= DISP_SetColor(HI_DRV_DISPLAY_1, &stCS);
    
    if(GAMMA_PQ_MODE_CLOSE == pstOptionChan1->u32GammaMode)
    {
        s32Ret |= DISP_SetGammaCtrl(HI_DRV_DISPLAY_1, pstOptionChan1->u32GammaMode, HI_FALSE);
    }
    else
    {
        s32Ret |= DISP_SetGammaCtrl(HI_DRV_DISPLAY_1, pstOptionChan1->u32GammaMode, HI_TRUE);
    }
    
    //set acc
    if(ACC_PQ_MODE_CLOSE == pstOptionChan1->u32DynamicContrast)
    {
        pstPqParam->stPQCoef.stAccCoef.stPqAccCtrl.u32Acc_en = HI_FALSE;
    }
    else
    {
        pstPqParam->stPQCoef.stAccCoef.stPqAccCtrl.u32Acc_en = HI_TRUE;
    }
    
    pstPqParam->stPQCoef.stAccCoef.stPqAccCtrl.u32Acc_mode = pstOptionChan1->u32DynamicContrast;
    pstVoFuncs->pfnWinUpdatePqData(PQ_CMD_VIRTUAL_ACC_CTRL, pstPqParam);

    //set acm
    if(ACM_PQ_MODE_CLOSE == pstOptionChan1->u32IntelligentColor)
    {
        pstPqParam->stPQCoef.stAcmCoef.stPqAcmCtrl.u32Acm_en = HI_FALSE;
    }
    else
    {
        pstPqParam->stPQCoef.stAcmCoef.stPqAcmCtrl.u32Acm_en = HI_TRUE;
    }
    
    pstPqParam->stPQCoef.stAcmCoef.stPqAcmCtrl.u32Acm_mode = pstOptionChan1->u32IntelligentColor;
    pstVoFuncs->pfnWinUpdatePqData(PQ_CMD_VIRTUAL_ACM_CTRL, pstPqParam);
    return s32Ret;
}
...
{
    HI_S32  s32Ret = HI_FAILURE;
    PQ_OPT_IMAGE_S *pstOptionImage = &(pstPqParam->stPqOption.stWorkPqOptCoef.stOptImage);
    //PQ_OPT_COMMON_S *pstOptionComm = &(pstPqParam->stPqOption.stWorkPqOptCoef.stOptCommon);
    PQ_GAMMA_COEF_S *pstGammaCoef = &(pstPqParam->stPQCoef.stGammaCoef);

    switch(u32UpdateType)
    {
        case PQ_CMD_VIRTUAL_OPTION_DISP0_MOD0:
        { 
            if(OPTION_MODE0 == pstOptionImage->u32OptionChan0Mode)
            {
               
                s32Ret = DRV_DISP_SetChan0Option(pstPqParam,OPTION_MODE0);
            }
            break;
        }
        case PQ_CMD_VIRTUAL_OPTION_DISP0_MOD1:
        {
            if(OPTION_MODE1 == pstOptionImage->u32OptionChan0Mode)
            {
                
                s32Ret = DRV_DISP_SetChan0Option(pstPqParam,OPTION_MODE1);
            }  
            break;
		...
		{
            if(OPTION_MODE2 == pstOptionImage->u32OptionChan0Mode)
            {
               
                s32Ret = DRV_DISP_SetChan0Option(pstPqParam,OPTION_MODE2);
            }
            break;
        }			
        case PQ_CMD_VIRTUAL_OPTION_DISP0_MOD3:
		{	
            if(OPTION_MODE3 == pstOptionImage->u32OptionChan0Mode)
            {                
                s32Ret = DRV_DISP_SetChan0Option(pstPqParam,OPTION_MODE3);
            }
            break;
        }
        case PQ_CMD_VIRTUAL_OPTION_DISP0_UNF:
        {
            s32Ret = DRV_DISP_SetChan0Option(pstPqParam,pstOptionImage->u32OptionChan0Mode);
            break;
        }
        case PQ_CMD_VIRTUAL_OPTION_DISP1_MOD0:
		{
            if(OPTION_MODE0 == pstOptionImage->u32OptionChan1Mode)
            {
                
                s32Ret = DRV_DISP_SetChan1Option(pstPqParam,OPTION_MODE0);
            }
            break;
        }
        case PQ_CMD_VIRTUAL_OPTION_DISP1_MOD1:
		{	
            if(OPTION_MODE1 == pstOptionImage->u32OptionChan1Mode)
            {
                
                s32Ret = DRV_DISP_SetChan1Option(pstPqParam,OPTION_MODE1);
            }
            break;
        }			
        case PQ_CMD_VIRTUAL_OPTION_DISP1_MOD2:
		{
            if(OPTION_MODE2 == pstOptionImage->u32OptionChan1Mode)
            {
               
                s32Ret = DRV_DISP_SetChan1Option(pstPqParam,OPTION_MODE2);
            } 
            break;
        }			
        case PQ_CMD_VIRTUAL_OPTION_DISP1_MOD3:
		{
            if(OPTION_MODE3 == pstOptionImage->u32OptionChan1Mode)
            {
                
                s32Ret = DRV_DISP_SetChan1Option(pstPqParam,OPTION_MODE3);
            }
            break;
        }
        case PQ_CMD_VIRTUAL_OPTION_DISP1_UNF:
        {
            ...
            s32Ret = DRV_DISP_SetChan1Option(pstPqParam,pstOptionImage->u32OptionChan1Mode);
            break;
	    ...
	    {  
            s32Ret = DISP_UpdateGamma(GAMMA_MODE_RGB, &(pstGammaCoef->stPqGammaRgbMode));	
	        break;
        }
	    ...
	    {			
            s32Ret = DISP_UpdateGamma(GAMMA_MODE_YUV, &(pstGammaCoef->stPqGammaYuvMode));
	        break;
        }/*
        case PQ_CMD_VIRTUAL_GAMMA_CTRL_DISPLAY0:	
	    {	
	     s32Ret = DISP_SetGammaCtrl(HI_DRV_DISPLAY_0, pstGammaCoef->stPqGammaDisp0Ctrl.u32GammaMode, pstGammaCoef->stPqGammaDisp0Ctrl.u32GammaEnable);
	     break;		  
        }	
        case PQ_CMD_VIRTUAL_GAMMA_CTRL_DISPLAY1:	
	    {	

	     s32Ret = DISP_SetGammaCtrl(HI_DRV_DISPLAY_1, pstGammaCoef->stPqGammaDisp1Ctrl.u32GammaMode, pstGammaCoef->stPqGammaDisp1Ctrl.u32GammaEnable);
	     break;		  
        }*/	
	    default:
	    {        	  	
