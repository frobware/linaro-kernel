...
#ifndef __DISP_PLATFORM_BOOT__
HI_S32 DispProcessPqParam(HI_DRV_DISPLAY_E enDisp)
{
    PQ_PARAM_S* pstPqParam = HI_NULL;
    PQ_OPT_IMAGE_S *pstPqOptImage = HI_NULL;
    PQ_OPT_CHAN0_S *pstPqOptChan0 = HI_NULL;
    PQ_OPT_CHAN1_S *pstPqOptChan1 = HI_NULL;
    ...
    s32Ret = HI_DRV_MODULE_GetFunction(HI_ID_PQ, (HI_VOID**)&pstPqFuncs);
    if ((s32Ret != HI_SUCCESS) || (HI_NULL == pstPqFuncs ))
    {
        DISP_WARN("Get PQ_EXPORT_FUNC_S failed\r\n");
    }
    else
    {
        ...
        pstPqOptImage = &(pstPqParam->stPqOption.stWorkPqOptCoef.stOptImage);
        pstPqOptChan0 = &(pstPqParam->stPqOption.stWorkPqOptCoef.stOptChan0[pstPqOptImage->u32OptionChan0Mode]);
        pstPqOptChan1 = &(pstPqParam->stPqOption.stWorkPqOptCoef.stOptChan1[pstPqOptImage->u32OptionChan1Mode]);

        s32Ret = DISP_UpdateGamma(GAMMA_MODE_RGB, &(pstPqParam->stPQCoef.stGammaCoef.stPqGammaRgbMode));
        s32Ret |= DISP_UpdateGamma(GAMMA_MODE_YUV, &(pstPqParam->stPQCoef.stGammaCoef.stPqGammaYuvMode));


        if(HI_DRV_DISPLAY_0 == enDisp)
        {
            if(GAMMA_PQ_MODE_CLOSE == pstPqOptChan0->u32GammaMode)
            {
                s32Ret |= DISP_SetGammaCtrl(enDisp, pstPqOptChan0->u32GammaMode, HI_FALSE);
            }
            else
            {
                s32Ret |= DISP_SetGammaCtrl(enDisp, pstPqOptChan0->u32GammaMode, HI_TRUE);
            }
        }
        else
        if(HI_DRV_DISPLAY_1 == enDisp)
        {
            if(GAMMA_PQ_MODE_CLOSE == pstPqOptChan1->u32GammaMode)
            {
                s32Ret |= DISP_SetGammaCtrl(enDisp, pstPqOptChan1->u32GammaMode, HI_FALSE);
            }
            else
            {
                s32Ret |= DISP_SetGammaCtrl(enDisp, pstPqOptChan1->u32GammaMode, HI_TRUE);
            }
        }
...
HI_S32 DispParserPqParam(HI_DRV_DISPLAY_E enDisp, HI_DRV_DISP_SETTING_S* pstSetting)
{
    PQ_PARAM_S* pstPqParam = HI_NULL;
    PQ_OPT_IMAGE_S *pstPqOptImage = HI_NULL;
    PQ_OPT_CHAN0_S *pstPqOptChan0 = HI_NULL;
    ...
    else
    {
        ...
        if(HI_DRV_DISPLAY_0 == enDisp)
        {
            pstSetting->stColor.u32Bright = pstPqOptChan0->u32Brightness;
            pstSetting->stColor.u32Contrst = pstPqOptChan0->u32Contrast;
            pstSetting->stColor.u32Satur = pstPqOptChan0->u32Saturation;
            pstSetting->stColor.u32Hue = pstPqOptChan0->u32Hue;
        }
        else
        if(HI_DRV_DISPLAY_1 == enDisp)
        {
            pstSetting->stColor.u32Bright = pstPqOptChan1->u32Brightness;
            pstSetting->stColor.u32Contrst = pstPqOptChan1->u32Contrast;
            pstSetting->stColor.u32Satur = pstPqOptChan1->u32Saturation;
            pstSetting->stColor.u32Hue = pstPqOptChan1->u32Hue;
...
HI_S32 DispParserPqParam(HI_DRV_DISPLAY_E enDisp, HI_DRV_DISP_SETTING_S* pstSetting)
{
    ...
    s32Ret = HI_PQ_GetPQData(&pstPqParam);
...
HI_VOID DispInitDisplay(HI_DRV_DISPLAY_E enDisp)
{
    ...
    DispParserPqParam(enDisp, &stDefSetting);
        ...
        case DISP_PRIV_STATE_ENABLE:
            ...
            else
            {
                ...
                else
                {
                    DISP_ISR_SetEvent(pstDisp->enDisp, HI_DRV_DISP_C_VT_INT);
                    //DISP_ISR_SetDispInfo(pstDisp->enDisp, &pstDisp->stDispInfo);
                    //DispProcessPqParam(pstDisp->enDisp);
                }
            }
...
HI_S32 DISP_ExternSetEnable(HI_DRV_DISPLAY_E enDisp, HI_BOOL bEnable)
{
    ...
    if (bEnable)
    {
        ...
        if ((HI_DRV_DISPLAY_BUTT != DISPGetIntfChannel(HI_DRV_DISP_INTF_CVBS0))
            || (HI_DRV_DISPLAY_BUTT != DISPGetIntfChannel(HI_DRV_DISP_INTF_YPBPR0)))
        {
    ...
    {
        if ((HI_DRV_DISPLAY_BUTT != DISPGetIntfChannel(HI_DRV_DISP_INTF_HDMI0)) && !DispGetHdmiFunction())
         {
...
HI_S32 DISPCheckCustomTiming(HI_DRV_DISP_TIMING_S* pstTiming)
{
    ...
    if ( ( !pstTiming->u32ClkPara0 ) ||  ( !pstTiming->u32ClkPara0 ))
    {
...
HI_S32 DISP_SetGammaCtrl(HI_DRV_DISPLAY_E eDisp, GAMMA_MODE_E enGammaMode, HI_BOOL bEnable)
{
    ...
    pstDisp->stSetting.stColor.bGammaEnable = bEnable;
...
HI_S32  DRV_DISP_O5_SetBright(HI_DRV_DISPLAY_E enDisp, HI_U32 CscValue)
{
    HI_DRV_DISP_COLOR_SETTING_S stCs;

    (HI_VOID)DISP_GetColor(enDisp, &stCs);

    stCs.u32Bright =  CscValue;
    
    (HI_VOID)DISP_SetColor(enDisp, &stCs);
    
    return HI_SUCCESS;
}
HI_S32  DRV_DISP_O5_GetBright(HI_DRV_DISPLAY_E enDisp, HI_U32 *pCscValue)
{
    HI_DRV_DISP_COLOR_SETTING_S stCs;
    
    memset((void*)&stCs, 0, sizeof(HI_DRV_DISP_COLOR_SETTING_S));
    (HI_VOID)DISP_GetColor(enDisp, &stCs);

    *pCscValue = stCs.u32Bright;
    
    return HI_SUCCESS;
}

HI_S32  DRV_DISP_O5_SetContrast(HI_DRV_DISPLAY_E enDisp, HI_U32 CscValue)
{
    HI_DRV_DISP_COLOR_SETTING_S stCs;

    (HI_VOID)DISP_GetColor(enDisp, &stCs);

    stCs.u32Contrst=  CscValue;
    
    (HI_VOID)DISP_SetColor(enDisp, &stCs);
    
    return HI_SUCCESS;
}
HI_S32  DRV_DISP_O5_GetContrast(HI_DRV_DISPLAY_E enDisp, HI_U32 *pCscValue)
{
    HI_DRV_DISP_COLOR_SETTING_S stCs;
...
EXPORT_SYMBOL(DRV_DISP_O5_SetBright);
EXPORT_SYMBOL(DRV_DISP_O5_GetBright);
EXPORT_SYMBOL(DRV_DISP_O5_SetContrast);
EXPORT_SYMBOL(DRV_DISP_O5_GetContrast);
