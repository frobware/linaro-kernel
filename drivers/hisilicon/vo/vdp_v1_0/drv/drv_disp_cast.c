...
DEFINE_SPINLOCK(g_threadCast_Lock);
...
static HI_S32 Cast_KThread_Release(HI_VOID *pstCastmp)
{
    HI_U32 i = 0;
    ...
    memset((void*)&pstFrame, 0, sizeof(HI_DRV_VIDEO_FRAME_S));
    ...
    {        
        ...
        spin_lock(&g_threadCast_Lock);         
        for (i = 0; i < DISP_CAST_BUFFER_MAX_NUMBER; i++)
        {
            if (atomic_read(&g_stReleasePtrArray[i].bFull))
            {
                /*check whether the cast output size changed.*/
                nRet = BP_GetFrame(&pstCast->stBP,  
                            g_stReleasePtrArray[i].u32BufID,
                            &pstFrame);
                
                if (nRet)
                   break;

                if (Cast_CheckOutputSizeChange(pstCast, &pstFrame))
                {
                ...
                atomic_set(&g_stReleasePtrArray[i].bFull, 0);
            }
        }
        
        spin_unlock(&g_threadCast_Lock);
        msleep(10);        
    }    
...
{    
    pstCast->kThreadReleaseFrame = kthread_create(Cast_KThread_Release, pstCast, "HI_DISP_CastRelease"); 
...
HI_S32 DISP_CastAddFullNode2ReallocateList(HI_DRV_VIDEO_FRAME_S *pstCastFrame, HI_U32 u32BufId)
{    
    HI_U32 i = 0;   
    ...
    for (i = 0; i < DISP_CAST_BUFFER_MAX_NUMBER; i++)
    {
        if (!atomic_read(&g_stReleasePtrArray[i].bFull))
        {
            ...
            g_stReleasePtrArray[i].bInternalRelease = HI_TRUE;
            atomic_set(&g_stReleasePtrArray[i].bFull, 1);
            break;
        }
    }
    return HI_SUCCESS;
...
HI_S32 DISP_CastReleaseFrame(HI_HANDLE cast_ptr, HI_DRV_VIDEO_FRAME_S *pstCastFrame)
{
    ...
    for (i = 0; i < DISP_CAST_BUFFER_MAX_NUMBER; i++)
    {
        ...
        {
            g_stReleasePtrArray[i].u32BufID = u32BufId;
            g_stReleasePtrArray[i].bInternalRelease = HI_FALSE;
            atomic_set(&g_stReleasePtrArray[i].bFull, 1);
            break;
        }
    }   
...
HI_VOID DISP_CastPushFrame(HI_HANDLE cast_ptr, const HI_DRV_DISP_CALLBACK_INFO_S *pstInfo)
{
    ...
    if (!(spin_trylock(&g_threadCast_Lock)))
    {
        return;
    }
...
HI_VOID DISP_CastCB_GenarateFrame(HI_HANDLE cast_ptr, const HI_DRV_DISP_CALLBACK_INFO_S *pstInfo)
{
    ...
    if (pstInfo->eEventType == HI_DRV_DISP_C_VT_INT)
    {
        ...
        {
            DISP_CastAddFullNode2ReallocateList(&stCurFrame, u32BufId);            
            nRet = Cast_GetFrame(pstCast, &u32BufId, 0, &stCurFrame);
            if (nRet)
            {
                spin_unlock(&g_threadCast_Lock);
