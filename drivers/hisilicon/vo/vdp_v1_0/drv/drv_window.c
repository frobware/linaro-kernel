 ...
 */
HI_S32 Revise_OutOfScreenWin_OutRect(HI_RECT_S *pstInRect, 
...
/*principly, we only use vpss to zme out or in.  but video layer only support a limit input size.
   so in large scale zme, we should do a 2-class zme: vpss and vdp both do scaler.*/
HI_S32 WinSizeAssigned_BettweenTwoZme(WINDOW_S *pstWin, HI_RECT_S *pstOutRect, HI_RECT_S *pstReadRect)
{
    ...
    {
        if (pstReadRect->s32Width > u32LayerSupportWidthMax)
        {
            pstOutRect->s32Width  = u32LayerSupportWidthMax;
        }
        
        if (pstReadRect->s32Height > u32LayerSupportHeightMax)
        {
            pstOutRect->s32Height = u32LayerSupportHeightMax;
        }
    }
    else
    {
        if((pstReadRect->s32Width > u32LayerSupportWidthMax)||(pstReadRect->s32Height > u32LayerSupportHeightMax))
        {
            if(u32LayerSupportWidthMax*pstOutRect->s32Height > u32LayerSupportHeightMax*pstOutRect->s32Width)
            {                
                pstOutRect->s32Width = (pstOutRect->s32Width*u32LayerSupportHeightMax/pstOutRect->s32Height)&HI_WIN_IN_RECT_WIDTH_ALIGN;                
                pstOutRect->s32Height = u32LayerSupportHeightMax;
            }
            else
            {
                pstOutRect->s32Height = (pstOutRect->s32Height*u32LayerSupportWidthMax/pstOutRect->s32Width)&HI_WIN_IN_RECT_HEIGHT_ALIGN; 
                pstOutRect->s32Width = u32LayerSupportWidthMax;
            }
        }
    }
    
    if ((!pstOutRect->s32Height) || (!pstOutRect->s32Width))
    {
        pstOutRect->s32Width =   VIDEO_LAYER_SUPPORT_MIN_WIDTH;
        pstOutRect->s32Height =  VIDEO_LAYER_SUPPORT_MIN_HEIGHT;        
    }
...
HI_S32 WinSendAttrToSource(WINDOW_S *pstWin, HI_DISP_DISPLAY_INFO_S *pstDispInfo)
{
    ...
    if (pstWin->stCfg.stSource.pfSendWinInfo)
    {
       ...
       // stInfo.stScreen     = pstDispInfo->stFmtResolution; 
        ...
        WinSizeAssigned_BettweenTwoZme(pstWin,
                                       &stInfo.stOutRect,
                                       &stInfo.stOutRect);
...
HI_S32 WIN_ProcessPq(HI_VOID)
{
    HI_S32 s32Ret;        
    PQ_PARAM_S* pstPqParam = HI_NULL;
    PQ_EXPORT_FUNC_S* pstPqFuncs = HI_NULL;

    /*static HI_U32 su32times = 0;
    if(su32times >= 1)
        return 0;
    su32times++;*/
    
    (HI_VOID)HI_DRV_MODULE_GetFunction(HI_ID_PQ, (HI_VOID**)&pstPqFuncs);
    ...
    else
    {
        ...
        {            
         ...
         HI_DRV_WIN_UpdatePqData(PQ_CMD_VIRTUAL_ACM_CTRL,pstPqParam);
        HI_DRV_WIN_UpdatePqData(PQ_CMD_VIRTUAL_ACC_CTRL,pstPqParam);   
    }
...
HI_VOID ISR_WinConfigFrameMute(WINDOW_S *pstWin, HI_DRV_VIDEO_FRAME_S *pstFrame, const HI_DRV_DISP_CALLBACK_INFO_S *pstInfo)
{
     ...
     */
    (HI_VOID)Revise_OutOfScreenWin_OutRect(&stLayerPara.stIn, &stLayerPara.stVideo,
...
#if defined(CHIP_TYPE_hi3719mv100) || defined(CHIP_TYPE_hi3719mv100_a) || defined(CHIP_TYPE_hi3718mv100)
        case FIDELITY_SKN:
        case FIDELITY_ZDN:
            u32Index = 2;
            break;
...
HI_VOID ISR_WinConfigFrameNormal(WINDOW_S *pstWin, HI_DRV_VIDEO_FRAME_S *pstFrame, const HI_DRV_DISP_CALLBACK_INFO_S *pstInfo)
{
    ...
    memset(&stLayerPara,0x0,sizeof(WIN_HAL_PARA_S));
    stLayerPara.bZmeUpdate = HI_TRUE;     
    stLayerPara.en3Dmode = pstInfo->stDispInfo.eDispMode;
    stLayerPara.bRightEyeFirst = pstInfo->stDispInfo.bRightEyeFirst;
    ...
    if ((pstFrame->stDispRect.s32Width != stLayerPara.stDisp.s32Width)
            || (pstFrame->stDispRect.s32Height!= stLayerPara.stDisp.s32Height))
    {
        stLayerPara.bZmeUpdate = HI_TRUE;
    ...
    }
    {
        ...
        stLayerPara.stOriRect = pstPriv->stOriginImageRect;
        stLayerPara.u32Fidelity = pstPriv->u32Fidelity;
    }
    
#if 0
    if (uptmp == 1)
    {
         printk("11111--%d, %d, %d,%d!\n", stLayerPara.stVideo.s32X, stLayerPara.stVideo.s32Y,
            stLayerPara.stVideo.s32Width, stLayerPara.stVideo.s32Height);        
    }
#endif
    ...
    if (u32Fidelity)
    {
        if ((pstWin->stVideoTmpInfo.u32WinInWidth != pstFrame->ePixFormat)
            ...
            //||(pstWin->stVideoTmpInfo.u32DispWidth != stLayerPara.pstDispInfo->)
            //||(pstWin->stVideoTmpInfo.u32DispHeight != pstFrame->ePixFormat)
            )
        {
            //printk("up zme !\n");
...
#if defined(CHIP_TYPE_hi3719mv100) || defined(CHIP_TYPE_hi3719mv100_a) || defined(CHIP_TYPE_hi3718mv100)
    /*3719m not need transform    601 to 709 */
#else
       ...
       )
    {
        ...
        if (  (pstWin->enDisp == HI_DRV_DISPLAY_0)
            ...
            stColor.u32Satur  = pstInfo->stDispInfo.u32Satur;
#endif
 //       printk("disp%d----%d--\n",pstWin->enDisp,stColor.u32Satur);
