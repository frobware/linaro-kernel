...
HI_S32 DRV_WIN_ProcRead(struct seq_file *p, HI_VOID *v)
{
    ...
    */
    ...
    PROC_PRINT(p, "%-19s:%-20d", "bQuickoutMode", pstProcInfo->bQuickMode);
...
#define WIN_3D_VIDEO_MIN_DEPTH -50
    ...
    else if (0 == HI_OSAL_Strncmp(pArg1, "rd_door", strlen("rd_door_start")))
    {
...
{ 
	...
	nRet = VideoLayer_GetFunction(&stVLayerFunc);
    
    if (nRet)
    {
        WIN_WARN("VideoLayer_GetFunction failed in %s!\n", __FUNCTION__);
        return HI_FAILURE;
    }   
    ...
    switch(u32UpdateType)
    {
        ...
        {
            stVLayerFunc.PF_UpdateACM(&(pstPqParam->stPQCoef.stAcmCoef));
			break;
        }  
        ...
        {
            stVLayerFunc.PF_UpdateACC(&(pstPqParam->stPQCoef.stAccCoef));
			break;
