...
const CSC_TABLE_S  VDPCSCTable_YCbCr601L_to_YCbCr709L =
{1024,  -118, -213,
    0,  1043,  117,
    0,    77, 1050};

/* YCbCr_709L (i.e. HD) -> YCbCr_601L (i.e. SD) */
const CSC_TABLE_S  VDPCSCTable_YCbCr709L_to_YCbCr601L =
{1024,  102,  196,
    0, 1014, -113,
    0,  -74, 1007};
...
const CSC_TABLE_S  VDPCSCTable_sRGBF_to_YCbCr709L =
{ 188,   629,  63,
 -103, -347, 450,
...
const CSC_TABLE_S  VDPCSCTable_sRGBF_to_YCbCr601L =
{ 264,  516, 100,
 -152, -298, 450,
...
const CSC_TABLE_S  VDPCSCTable_YCbCr709L_to_sRGBF =
{1192,    0,  1836,
 1192, -218,  -547,
...
const CSC_TABLE_S  VDPCSCTable_YCbCr601L_to_sRGBF =
{1192,    0, 1634,
 1192, -400, -833,
...
**************************************************/
/* identity matrix */
const CSC_TABLE_S VDPCSCTable_Identity = 
{1024,    0,    0,
    0, 1024,    0,
    0,    0, 1024};
...
const CSC_TABLE_S *CSC_MatrixTbl[][4] =
{
    /* ALG_CS_eUnknown = 0 */
    {
        &VDPCSCTable_Identity              ,    /* Identity */
        &VDPCSCTable_Identity              ,    /* Identity */
        &VDPCSCTable_Identity              ,    /* Identity */
        &VDPCSCTable_Identity                   /* Identity */
    },
    ...
    /* ALG_CS_eYCbCr_709 = 1 */
    {
        &VDPCSCTable_Identity              ,    /* YCbCr709 -> unknown */
        &VDPCSCTable_Identity              ,    /* YCbCr709 -> YCbCr709 */
        &VDPCSCTable_YCbCr709L_to_YCbCr601L,    /* YCbCr709 -> YCbCr601 */
        &VDPCSCTable_YCbCr709L_to_sRGBF         /* YCbCr709 -> sRGB */
    },

    /* ALG_CS_eYCbCr_601 = 2 */
    {
        &VDPCSCTable_Identity              ,    /* YCbCr601 -> unknown */
        &VDPCSCTable_YCbCr601L_to_YCbCr709L,    /* YCbCr601 -> YCbCr709 */
        &VDPCSCTable_Identity              ,    /* YCbCr601 -> YCbCr601 */
        &VDPCSCTable_YCbCr601L_to_sRGBF         /* YCbCr601 -> sRGB */
    },


    /* ALG_CS_eRGB_709 = 3 */
    {
        &VDPCSCTable_Identity              ,    /* sRGB -> unknown */
        &VDPCSCTable_sRGBF_to_YCbCr709L    ,    /* sRGB -> YCbCr709 */
        &VDPCSCTable_sRGBF_to_YCbCr709L    ,    /* sRGB -> YCbCr601 */
        &VDPCSCTable_Identity                   /* sRGB -> sRGB */
    }
...
//for RGB->YCbCr 
static HI_VOID CalcCscCoef_RGBtoYCbCr(ALG_CSC_DRV_PARA_S *pstCscDrvPara, CSC_TABLE_S *pMatrixOri, CSC_TABLE_S *pMatrixDst)
{
    ...
    pMatrixDst->s32C10 = (HI_S32)(((HI_S32)(pMatrixOri->s32C10 * VDPCOS_TABLE[u32Hue]) / 1000
                                  + (HI_S32)(pMatrixOri->s32C20 * VDPSIN_TABLE[u32Hue]) / 1000)
                                 * s32ContrstAdj * s32SatAdj * s32Rgain) / 1000000;

    pMatrixDst->s32C11 = (HI_S32)(((HI_S32)(pMatrixOri->s32C11 * VDPCOS_TABLE[u32Hue]) / 1000
                                  + (HI_S32)(pMatrixOri->s32C21 * VDPSIN_TABLE[u32Hue]) / 1000)
                                 * s32ContrstAdj * s32SatAdj * s32Ggain) / 1000000;

    pMatrixDst->s32C12 = (HI_S32)(((HI_S32)(pMatrixOri->s32C12 * VDPCOS_TABLE[u32Hue]) / 1000
                                  + (HI_S32)(pMatrixOri->s32C22 * VDPSIN_TABLE[u32Hue]) / 1000)
                                 * s32ContrstAdj * s32SatAdj * s32Bgain) / 1000000;

    pMatrixDst->s32C20 = (HI_S32)(((HI_S32)(pMatrixOri->s32C20 * VDPCOS_TABLE[u32Hue]) / 1000
                                  - (HI_S32)(pMatrixOri->s32C10 * VDPSIN_TABLE[u32Hue]) / 1000)
                                 * s32ContrstAdj * s32SatAdj *s32Rgain) / 1000000;

    pMatrixDst->s32C21 = (HI_S32)(((HI_S32)(pMatrixOri->s32C21 * VDPCOS_TABLE[u32Hue]) / 1000
                                  - (HI_S32)(pMatrixOri->s32C11 * VDPSIN_TABLE[u32Hue]) / 1000)
                                 * s32ContrstAdj * s32SatAdj * s32Ggain) / 1000000;

    pMatrixDst->s32C22 = (HI_S32)(((HI_S32)(pMatrixOri->s32C22 * VDPCOS_TABLE[u32Hue]) / 1000
                                  - (HI_S32)(pMatrixOri->s32C12 * VDPSIN_TABLE[u32Hue]) / 1000)
                                 * s32ContrstAdj * s32SatAdj * s32Bgain) / 1000000;
...
//计算色彩空间转换系数
//for YCbCr->RGB  
static HI_VOID CalcCscCoef_YCbCrtoRGB(ALG_CSC_DRV_PARA_S *pstCscDrvPara, CSC_TABLE_S *pMatrixOri, CSC_TABLE_S *pMatrixDst)
{
    HI_S32 s32ContrstAdj, s32SatAdj;
    HI_S32 s32Rgain, s32Ggain, s32Bgain;
    HI_U32 u32Hue;
    ...
    pMatrixDst->s32C01 = (HI_S32)(((HI_S32)(pMatrixOri->s32C01 * VDPCOS_TABLE[u32Hue]) / 1000
                                  - ((HI_S32)(pMatrixOri->s32C02 * VDPSIN_TABLE[u32Hue]))* s32SatAdj / 100000)
                                 * s32ContrstAdj * s32Rgain) / 10000;
    
    pMatrixDst->s32C02 = (HI_S32)(((HI_S32)(pMatrixOri->s32C01 * VDPSIN_TABLE[u32Hue]) / 1000
                                  + ((HI_S32)(pMatrixOri->s32C02 * VDPCOS_TABLE[u32Hue]))* s32SatAdj / 100000)
                                 * s32ContrstAdj * s32Rgain) / 10000;
    ...
    pMatrixDst->s32C12 = (HI_S32)(((HI_S32)(pMatrixOri->s32C11 * VDPSIN_TABLE[u32Hue]) / 1000
                                  + (HI_S32)(pMatrixOri->s32C12 * VDPCOS_TABLE[u32Hue]) / 1000)
                                 * s32ContrstAdj * s32SatAdj * s32Ggain) / 1000000;

    pMatrixDst->s32C20 = pMatrixOri->s32C20 * s32ContrstAdj * s32Bgain/ 10000;

    pMatrixDst->s32C21 = (HI_S32)((((HI_S32)(pMatrixOri->s32C21 * VDPCOS_TABLE[u32Hue]))* s32SatAdj/ 100000
                                  - (HI_S32)(pMatrixOri->s32C22 * VDPSIN_TABLE[u32Hue]) / 1000)
                                 * s32ContrstAdj * s32Bgain) / 10000;

    pMatrixDst->s32C22 = (HI_S32)((((HI_S32)(pMatrixOri->s32C21 * VDPSIN_TABLE[u32Hue]))* s32SatAdj/ 100000
                                  - (HI_S32)(pMatrixOri->s32C22 * VDPCOS_TABLE[u32Hue]) / 1000)
                                 * s32ContrstAdj * s32Bgain) / 10000;
...
//计算色彩空间转换系数
//for YCbCr->YCbCr 
static HI_VOID CalcCscCoef_YCbCrtoYCbCr(ALG_CSC_DRV_PARA_S *pstCscDrvPara, CSC_TABLE_S *pMatrixOri, CSC_TABLE_S *pMatrixDst)
{
    HI_S32 s32ContrstAdj, s32SatAdj;
    HI_U32 u32Hue;
    ...
    pMatrixDst->s32C10 = (HI_S32)(((HI_S32)(pMatrixOri->s32C10
                                           * VDPCOS_TABLE[u32Hue]) / 1000
                                  + (HI_S32)(pMatrixOri->s32C20
                                             * VDPSIN_TABLE[u32Hue]) / 1000) * s32ContrstAdj * s32SatAdj) / 10000;
...
//for RGB->RGB  
static HI_VOID CalcCscCoef_RGBtoRGB(ALG_CSC_DRV_PARA_S *pstCscDrvPara, CSC_TABLE_S *pMatrixOri, CSC_TABLE_S *pMatrixDst)
{
    HI_S32 s32ContrstAdj, s32SatAdj;
    HI_S32 s32Rgain, s32Ggain, s32Bgain;
    HI_U32 u32Hue;
    ...
    pMatrixDst->s32C00 = (HI_S32)(306 + (HI_S32)(242*VDPSIN_TABLE[u32Hue]*s32SatAdj)/100000 + (HI_S32)(718*VDPCOS_TABLE[u32Hue]*s32SatAdj)/100000)*s32ContrstAdj*s32Rgain/10000;
    pMatrixDst->s32C01 = (HI_S32)(601 + (HI_S32)(475*VDPSIN_TABLE[u32Hue]*s32SatAdj)/100000 - (HI_S32)(601*VDPCOS_TABLE[u32Hue]*s32SatAdj)/100000)*s32ContrstAdj*s32Ggain/10000;
    pMatrixDst->s32C02 = (HI_S32)(117 - (HI_S32)(718*VDPSIN_TABLE[u32Hue]*s32SatAdj)/100000 - (HI_S32)(117*VDPCOS_TABLE[u32Hue]*s32SatAdj)/100000)*s32ContrstAdj*s32Bgain/10000;
	
    pMatrixDst->s32C10 = (HI_S32)(306 - (HI_S32)(299*VDPSIN_TABLE[u32Hue]*s32SatAdj)/100000 - (HI_S32)(306*VDPCOS_TABLE[u32Hue]*s32SatAdj)/100000)*s32ContrstAdj*s32Rgain/10000;
    pMatrixDst->s32C11 = (HI_S32)(601 - (HI_S32)(95*VDPSIN_TABLE[u32Hue]*s32SatAdj)/100000 + (HI_S32)(423*VDPCOS_TABLE[u32Hue]*s32SatAdj)/100000)*s32ContrstAdj*s32Ggain/10000;
    pMatrixDst->s32C12 = (HI_S32)(117 + (HI_S32)(394*VDPSIN_TABLE[u32Hue]*s32SatAdj)/100000 - (HI_S32)(117*VDPCOS_TABLE[u32Hue]*s32SatAdj)/100000)*s32ContrstAdj*s32Bgain/10000;

    pMatrixDst->s32C20 =(HI_S32)(306 + (HI_S32)(906*VDPSIN_TABLE[u32Hue]*s32SatAdj)/100000 - (HI_S32)(306*VDPCOS_TABLE[u32Hue]*s32SatAdj)/100000)*s32ContrstAdj*s32Rgain/10000;
    pMatrixDst->s32C21 = (HI_S32)(601 - (HI_S32)(760*VDPSIN_TABLE[u32Hue]*s32SatAdj)/100000 - (HI_S32)(601*VDPCOS_TABLE[u32Hue]*s32SatAdj)/100000)*s32ContrstAdj*s32Ggain/10000;
    pMatrixDst->s32C22 = (HI_S32)(117 - (HI_S32)(146*VDPSIN_TABLE[u32Hue]*s32SatAdj)/100000 + (HI_S32)(907*VDPCOS_TABLE[u32Hue]*s32SatAdj)/100000)*s32ContrstAdj*s32Bgain/10000;
  
    return;
}

HI_VOID VDPGetCscType(HI_DRV_COLOR_SPACE_E eCSDrv, ALG_CS_S *pstCsAlg)
{
    switch (eCSDrv)
    {
...
static HI_VOID CalcCscMatrix(ALG_CSC_DRV_PARA_S *pstCscDrvPara, ALG_CSC_RTL_PARA_S *pstCscRtlPara)
{
    CSC_TABLE_S *pCSCMatrixOri = NULL; 
    CSC_TABLE_S stCscTbl={0};
    ALG_CS_S stAlgCSIn, stAlgCSOut;

    VDPGetCscType(pstCscDrvPara->eInputCS, &stAlgCSIn);
    VDPGetCscType(pstCscDrvPara->eOutputCS, &stAlgCSOut);

    pCSCMatrixOri = (CSC_TABLE_S *)CSC_MatrixTbl[stAlgCSIn.eCsType][stAlgCSOut.eCsType];
    ...
    //RANGE_FULL -> RANGE_LMTD
    if((stAlgCSIn.eCsRange == ALG_CS_RANGE_FULL)&&(stAlgCSOut.eCsRange == ALG_CS_RANGE_LMTD))
    {   //RGB OUT
        if(stAlgCSOut.eCsType ==ALG_CS_eRGB_709)
        {
            ;
        }
        else//YUV OUT
        {
            ...
            pstCscRtlPara->s32CscCoef_00 = (HI_S32)((pstCscRtlPara->s32CscCoef_00)*219/255);
            pstCscRtlPara->s32CscCoef_01 = (HI_S32)((pstCscRtlPara->s32CscCoef_01)*219/255);
            pstCscRtlPara->s32CscCoef_02 = (HI_S32)((pstCscRtlPara->s32CscCoef_02)*219/255);
            pstCscRtlPara->s32CscCoef_10 = (HI_S32)((pstCscRtlPara->s32CscCoef_10)*224/255);
            pstCscRtlPara->s32CscCoef_11 = (HI_S32)((pstCscRtlPara->s32CscCoef_11)*224/255);
            pstCscRtlPara->s32CscCoef_12 = (HI_S32)((pstCscRtlPara->s32CscCoef_12)*224/255);
            pstCscRtlPara->s32CscCoef_20 = (HI_S32)((pstCscRtlPara->s32CscCoef_20)*224/255);
            pstCscRtlPara->s32CscCoef_21 = (HI_S32)((pstCscRtlPara->s32CscCoef_21)*224/255);
            pstCscRtlPara->s32CscCoef_22 = (HI_S32)((pstCscRtlPara->s32CscCoef_22)*224/255);
        }
    }
    //RANGE_LMTD -> RANGE_FULL
    else if((stAlgCSIn.eCsRange == ALG_CS_RANGE_LMTD)&&(stAlgCSOut.eCsRange == ALG_CS_RANGE_FULL))
    {   //RGB
        if(stAlgCSOut.eCsType ==ALG_CS_eRGB_709)
        {
            ;
        ...
        {
            pstCscRtlPara->s32CscCoef_00 = (HI_S32)((pstCscRtlPara->s32CscCoef_00)*255/219);
            pstCscRtlPara->s32CscCoef_01 = (HI_S32)((pstCscRtlPara->s32CscCoef_01)*255/219);
            pstCscRtlPara->s32CscCoef_02 = (HI_S32)((pstCscRtlPara->s32CscCoef_02)*255/219);
            pstCscRtlPara->s32CscCoef_10 = (HI_S32)((pstCscRtlPara->s32CscCoef_10)*255/224);
            pstCscRtlPara->s32CscCoef_11 = (HI_S32)((pstCscRtlPara->s32CscCoef_11)*255/224);
            pstCscRtlPara->s32CscCoef_12 = (HI_S32)((pstCscRtlPara->s32CscCoef_12)*255/224);
            pstCscRtlPara->s32CscCoef_20 = (HI_S32)((pstCscRtlPara->s32CscCoef_20)*255/224);
            pstCscRtlPara->s32CscCoef_21 = (HI_S32)((pstCscRtlPara->s32CscCoef_21)*255/224);
            pstCscRtlPara->s32CscCoef_22 = (HI_S32)((pstCscRtlPara->s32CscCoef_22)*255/224);
        }
    }
}
...
static HI_VOID CalcCscDc(ALG_CSC_DRV_PARA_S *pstCscDrvPara, ALG_CSC_RTL_PARA_S *pstCscRtlPara)
{
