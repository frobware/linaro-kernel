...
SINT32 iMediaSoftDEC_Create(iMediaSDEC_CTX_S  *pCtx, MEM_DESC_S *pstInstMem, VID_STD_E eVidStd)
{
	...
	default:
		...
		(pVfmwToSvdecFun->pfnVfmwDprint)(PRN_ALWS,"iMedia soft codec can NOT support eVidStd=%d!\n", eVidStd);
		#endif
	...
	if (0 != (s32Ret = IMedia_Viddec_QueryMemSize(iMediaCodecType, &stMem, &s32MemNeedSize)))
	{
	    ...
	    #ifndef HI_ADVCA_FUNCTION_RELEASE
		(pVfmwToSvdecFun->pfnVfmwDprint)(PRN_ALWS, "ERROR: IMedia_Viddec_QueryMemSize return %d\n", s32Ret);
		#endif
        return iMediaSDEC_ERR;
	}
    ...
    {
      	#ifndef HI_ADVCA_FUNCTION_RELEASE
        (pVfmwToSvdecFun->pfnVfmwDprint)(PRN_ALWS,"ERROR: Create iMedia codec error, mem need(%d) > have(%d)\n", s32MemNeedSize, pstInstMem->Length);
        #endif
		return iMediaSDEC_ERR;
	}
