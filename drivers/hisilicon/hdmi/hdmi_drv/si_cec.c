...
HI_U8 SiI_CEC_SetCommand( SiI_CEC_t * pSiI_CEC )
{
    ...
    HI_U32 u32Index = 0;
    HI_INFO_HDMI("***SiI_CEC_SetCommand start ***\n");
    // Clear Tx Buffer
    ...
    {
        HI_CEC_HDMI("0x%02x ",(int)pSiI_CEC->bOperand[u32Index]);
    }
...
#endif
    HI_INFO_HDMI("***SiI_CEC_SetCommand stop ***\n");
    return error;
}
...
HI_U8 SI_CEC_GetCommand( SiI_CEC_t * pSiI_CEC )
{
    ...
    if ( !(pSiI_CEC->bCount & BIT_MSG_ERROR) )
    {
        ...
        HI_CEC_HDMI("RX HDR:0x%02X,OpCode:0x%02X, Count:%d, \
