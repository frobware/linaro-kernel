...
void SI_PrintAuthState(void)
{
    ...
    case RIs_ARE_MISSMATCH:
        ...
        break;
    case AUTHENTICATED:
        HDMI_PRINT("Authenticated");
        break;
    case REPEATER_AUTH_REQ:
        HDMI_PRINT("Rptr Auth. Req");
        break;
    case REQ_SHA_CALC:
        HDMI_PRINT("Req SHA calc");
        break;
    case REQ_SHA_HW_CALC:
        HDMI_PRINT("Req SHA HW calc");
        break;
    case FAILED_ViERROR:
        HDMI_PRINT("V Err");
        break;
    case REAUTHENTATION_REQ:
        HDMI_PRINT("Reauthentication Req");
        break;
    case REQ_AUTHENTICATION:
        HDMI_PRINT("Req authentication");
        break;
    default:
        HDMI_PRINT("Unknown %02i",AuthState);
        break;
    }
...
HI_U8 SI_TimerHandler(void)
{
    ...
    if((OutputState == CABLE_PLUGIN_HDMI_OUT) || (OutputState == CABLE_PLUGIN_DVI_OUT))
    {
        ...
        if( (AuthState == REPEATER_AUTH_REQ) || ( AuthState == REQ_SHA_CALC ) || ( AuthState == REQ_SHA_HW_CALC ))
        {
            ...
            {
            	HI_INFO_HDMI("hdcp protocol 2:come to SHA_HW_Handler\n");
                ...
                {
                    HI_ERR_HDMI("SHA_HW_Handler(hdcp protocol 2) fail\n");
                    //DRV_HDMI_NotifyEvent(HI_UNF_HDMI_EVENT_HDCP_FAIL);
                }
