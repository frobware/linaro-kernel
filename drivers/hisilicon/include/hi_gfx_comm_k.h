...
#include "hi_drv_module.h"
   #if defined(CONFIG_CHIP_3712_VERSION) || defined(CONFIG_CHIP_3716MV300_VERSION) || defined(CONFIG_CHIP_3716CV100_VERSION) || defined(CONFIG_CHIP_3716HV100_VERSION)
...
#if !defined(CONFIG_CHIP_3712_VERSION) && !defined(CONFIG_CHIP_3716MV300_VERSION) && !defined(CONFIG_CHIP_3716CV100_VERSION) && !defined(CONFIG_CHIP_3716HV100_VERSION) && !defined(CONFIG_CHIP_S40V300_VERSION)
...
static HI_U32 HI_GFX_AllocMem(HI_CHAR *pName, HI_CHAR* pZoneName, HI_U32 u32LayerSize)
{
	    ...
	    if (MMB_ADDR_INVALID != (addr = new_mmb(pName,  u32LayerSize, GFX_MMZ_ALIGN_BYTES, pZoneName)))
	    {  
			return addr;
	    }
	    else  if (MMB_ADDR_INVALID != (addr = new_mmb(pName,  u32LayerSize, GFX_MMZ_ALIGN_BYTES, "graphics")))
	    {
			return addr;    
	    }
	    else if (MMB_ADDR_INVALID != (addr = new_mmb(pName,  u32LayerSize, GFX_MMZ_ALIGN_BYTES, NULL)))
	    {
			return addr;    
	    }
	    else
