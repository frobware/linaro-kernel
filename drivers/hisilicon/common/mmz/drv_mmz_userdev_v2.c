...
#define error(s...)    do{ PRINTK_CA(KERN_ERR     "DRV_MMZ:%d.%s.%d: ", current->pid, __FUNCTION__, __LINE__); PRINTK_CA(s); }while(0)
#define warning(s...)  do{ PRINTK_CA(KERN_WARNING "DRV_MMZ:%d.%s.%d: ", current->pid, __FUNCTION__, __LINE__); PRINTK_CA(s); }while(0)
#if MMB_SHARE_DEBUG
#define info(s...)     do{ PRINTK_CA(KERN_INFO    "DRV_MMZ:%d.%s.%d: ", current->pid, __FUNCTION__, __LINE__); PRINTK_CA(s); }while(0)
//#define info(s...)     do{ HI_PRINT(KERN_INFO    "DRV_MMZ:%d.%s.%d: ", current->pid, __FUNCTION__, __LINE__); HI_PRINT(s); }while(0)
#else
#define info(s...)     
#endif
...
static int ioctl_mmb_user_unmap(struct file *file, unsigned int iocmd, struct mmb_info *pmi)
{
    ...
    if ((p = get_mmbinfo(pmi->phys_addr, pmu)) == NULL)  {
        ...
        info( "%s.%s unmap [0x%08lX, 0x%08lX, %d].[0x%08lX, 0x%08lX, %d].%d\n", __FUNCTION__, (pMapped->pid == pBase->pid)?"base":"copy",pMapped->phys_addr, pMapped->size,pMapped->pid, pBase->phys_addr, pBase->size,pBase->pid, current->pid);
