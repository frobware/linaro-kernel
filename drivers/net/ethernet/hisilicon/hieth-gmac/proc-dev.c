...
/* debug code */
static int set_suspend(struct device *dev,int eth_n)
{
...
/* debug code */
static int set_resume(struct device *dev)
{
...
int higmac_ioctl(struct net_device *net_dev, struct ifreq *rq, int cmd)
{
	...
	case SIOCSETSUSPEND:
		if (copy_from_user(&val, rq->ifr_data, sizeof(val)))
			return -EFAULT;
		return set_suspend(&(net_dev->dev),val);

	case SIOCSETRESUME:
		if (copy_from_user(&val, rq->ifr_data, sizeof(val)))
			return -EFAULT;
		return set_resume(&(net_dev->dev));
