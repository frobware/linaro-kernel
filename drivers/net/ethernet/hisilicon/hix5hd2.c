...
static void hix5hd2_hw_set_desc_queue_addr(struct hix5hd2_priv *priv)
{
...
static void hix5hd2_hw_init(struct hix5hd2_priv *priv)
{
	...
	val = RX_BQ_INT_THRESHOLD << QUEUE_RX_FQ_SHIFT |
		TX_RQ_INT_THRESHOLD << QUEUE_TX_BQ_SHIFT;
	...
	hix5hd2_hw_set_desc_queue_depth(priv);
	hix5hd2_hw_set_desc_queue_addr(priv);
}

static void hix5hd2_irq_enable(struct hix5hd2_priv *priv)
{
	writel_relaxed(RX_BQ_IN_INT | RX_BQ_IN_TIMEOUT_INT
			| TX_RQ_IN_INT | TX_RQ_IN_TIMEOUT_INT,
			priv->base + ENA_PMU_INT);
}
...
static void hix5hd2_hw_desc_enable(struct hix5hd2_priv *priv)
{
	writel_relaxed(0xF, priv->base + DESC_WR_RD_ENA);
}

static void hix5hd2_hw_desc_disable(struct hix5hd2_priv *priv)
{
...
static void hix5hd2_rx_refill(struct hix5hd2_priv *priv)
{
	...
	start = dma_cnt(readl(priv->base + RX_FQ_WR_ADDR));
	/* logic read pointer */
	end = dma_cnt(readl(priv->base + RX_FQ_RD_ADDR));
	...
	for (i = 0, pos = start; i < num; i++) {
		...
		if (dma_mapping_error(priv->dev, desc->data_buff_addr)) {
			dev_kfree_skb_any(skb);
			break;
		}

		priv->rx_skb[pos] = skb;
		desc->buffer_len = MAC_MAX_FRAME_SIZE - 1;
		desc->data_len = 0;
		desc->fl = 0;
...
static int hix5hd2_rx(struct net_device *dev, int limit)
{
	...
	start = dma_cnt(readl(priv->base + RX_BQ_RD_ADDR));
	/* logic write pointer */
	end = dma_cnt(readl(priv->base + RX_BQ_WR_ADDR));
...
static void hix5hd2_xmit_reclaim(struct net_device *dev)
{
	...
	/* software read */
	start = dma_cnt(readl(priv->base + TX_RQ_RD_ADDR));
	/* logic write */
	end = dma_cnt(readl(priv->base + TX_RQ_WR_ADDR));
...
static int hix5hd2_poll(struct napi_struct *napi, int budget)
{
	...
	work_done = hix5hd2_rx(dev, budget);
...
static irqreturn_t hix5hd2_interrupt(int irq, void *dev_id)
{
	...
	struct hix5hd2_priv *priv = netdev_priv(dev_id);
	struct net_device_stats *stats = &dev->stats;
	int ints = readl_relaxed(priv->base + RAW_PMU_INT);

	writel_relaxed(ints, priv->base + RAW_PMU_INT);

	if (unlikely(ints & RX_BQ_IN_TIMEOUT_INT)) {
		stats->rx_errors++;
		netdev_err(dev, "rx timeout\n");
	}

	if (unlikely(ints & TX_RQ_IN_TIMEOUT_INT)) {
		stats->tx_errors++;
		netdev_err(dev, "tx timeout\n");
	}

	if (likely(ints & (RX_BQ_IN_INT | TX_RQ_IN_INT))) {
...
static int hix5hd2_net_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	/* software write pointer */
	pos = dma_cnt(readl(priv->base + TX_BQ_WR_ADDR));
	...
	priv->tx_skb[pos] = skb;
	desc->buffer_len = skb->len;
	desc->data_len = skb->len;
	desc->fl = DESC_FL_FULL;
	desc->descvid = DESC_VLD_BUSY;

	pos = dma_ring_incr(pos, TX_DESC_NUM);
	writel(dma_byte(pos), priv->base + TX_BQ_WR_ADDR);
...
static void hix5hd2_free_dma_desc_rings(struct hix5hd2_priv *priv)
{
	...
	for (i = 0; i < RX_DESC_NUM; i++) {
		...
		dma_unmap_single(priv->dev, desc->data_buff_addr,
				 MAC_MAX_FRAME_SIZE, DMA_FROM_DEVICE);
		dev_kfree_skb_any(skb);
		priv->rx_skb[i] = NULL;
	}
...
static int hix5hd2_mdio_wait_ready(struct mii_bus *bus)
{
	...
	for (i = 0; readl(priv->base + MDIO_SINGLE_CMD) & MDIO_START; i++) {
		if (i == WAIT_TIMEOUT)
			return -ETIMEDOUT;
		udelay(1);
	}
...
static int hix5hd2_mdio_read(struct mii_bus *bus, int phy, int reg)
{
	...
	writel(MDIO_READ | phy << 8 | reg, priv->base + MDIO_SINGLE_CMD);
	ret = hix5hd2_mdio_wait_ready(bus);
	if (ret < 0)
		goto out;

	val = readl(priv->base + MDIO_RDATA_STATUS);
	if (val & MDIO_R_VALID) {
	...
	val = readl(priv->base + MDIO_SINGLE_DATA);
	ret = (val >> 16) & 0xFFFF;
out:
	return ret;
}
...
static int hix5hd2_mdio_write(struct mii_bus *bus, int phy, int reg, u16 val)
{
	...
	writel(val, priv->base + MDIO_SINGLE_DATA);
	writel(MDIO_WRITE | phy << 8 | reg, priv->base + MDIO_SINGLE_CMD);
	ret = hix5hd2_mdio_wait_ready(bus);
out:
	return ret;
}
...
MODULE_DESCRIPTION("HISILICON X5HD2 Ethernet driver");
MODULE_LICENSE("GPL v2");
MODULE_ALIAS("platform:hix5hd2-ether");
